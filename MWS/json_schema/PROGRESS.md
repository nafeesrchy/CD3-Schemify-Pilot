...
- sentinels: `-1` not_on_questionnaire (adopted verbatim from source, one specific reason) · `-999` missing_numeric (agent-decided catch-all for any other/unexplained gap) — the two are not mutually exclusive: a field with its own documented sentinel like `-1` still also carries `-999` for anything that sentinel doesn't explain · D020 (amends D003)
